...
public class TouristRepository {
    ...
    public int saveTouristAttractions(TouristAttraction attraction) {
        ...
        //SQL strings
        String attractionSQLString = "INSERT INTO touristattractions (name, description, city) VALUES (?, ?, ?)";
        String tagIDSQLString = "SELECT id FROM tags WHERE name = ?";
        String attractionTagsSQLString = "INSERT INTO attractiontags (tourist_attraction_id, tag_id) VALUES (?, ?)";

        int touristAttractionId;
        //conn.setAutoCommit(false);

        // Insert the attraction into the database
        try (PreparedStatement statement = conn.prepareStatement(attractionSQLString, Statement.RETURN_GENERATED_KEYS)) {
            ...
            handleTagsTable(attraction, tagIDSQLString, attractionTagsSQLString, touristAttractionId);
    ...
    public void handleTagsTable(TouristAttraction attraction, String tagIDSQLString, String attractionTagsSQLString, int touristAttractionId){
            ...
            for (String tag : attraction.getTags().split(",")) {
                ...
                try (ResultSet resultSet = tagIDStatement.executeQuery()) {
                    if (resultSet.next()) {//if resultset not empty:
                        int tagId = resultSet.getInt("id");//set tagid =
